...
class MainWindow(QMainWindow, Ui_mainWindow):
    def __init__(self, parent=None):
        ...
        self.detectpushButton.clicked.connect(self.detectPage)
        self.showpushButton_2.clicked.connect(self.showPage)
        ...
        #
        # filename = "D:\OBJ\模型11\cloud 1_group 1.obj"
    ...
    def open_model(self):
        ...
        config = json.load(open(config_file, 'r', encoding='utf-8'))
        open_fold = config['model_open_fold']
        self.filename, _ = QFileDialog.getOpenFileName(self, '选择模型文件', open_fold, "Pic File(*.obj)")

        # 保存上次打开的文件目录
        config['model_open_fold'] = os.path.dirname(self.filename)
        config_json = json.dumps(config, ensure_ascii=False, indent=2)
        with open(config_file, 'w', encoding='utf-8') as f:
            f.write(config_json)

        # 打开文件夹
        # self.fileBoxPath = QFileDialog.getOpenFileNames()
        # self.filenameJPG, _ =QFileDialog.getOpenFileName(self, '选取纹理图片',open_fold, "Pic File(*.png)")
        # self.filenameJPG = 'D:/OBJ/模型8/Ycloud1_group10.png'
        config_file = 'config/fold.json'
        config = json.load(open(config_file, 'r', encoding='utf-8'))
        open_fold = config['model_open_fold']
        self.filenameJPG = open_fold + '/'+'cloud1_group10.png'
        if self.filename:
            self.ren = vtk.vtkRenderer()
            self.ren.SetBackground(0.28, 0.26, 0.26)
            self.vtkWidget.GetRenderWindow().AddRenderer(self.ren)
            self.iren = self.vtkWidget.GetRenderWindow().GetInteractor()

            self.dirreader = vtk.vtkOBJReader()
            self.dirreader.SetFileName(self.filename)
            self.dirreader.Update()

            self.dirmapper = vtk.vtkPolyDataMapper()
            self.dirmapper.SetInputConnection(self.dirreader.GetOutputPort())



            # 设置纹理
            self.reader = vtk.vtkPNGReader()
            self.reader.SetFileName(self.filenameJPG)
            print(self.filename)
            ...
            # Create an actor
            self.diractor = vtk.vtkActor()
            self.diractor.SetMapper(self.dirmapper)
            self.diractor.SetTexture(self.texture)

            self.ren.AddActor(self.diractor)
            ...
            # self.ren.ResetCamera()
            # self.ren.SetActiveCamera()
            # camera = self.ren.GetActiveCamera()
            # camera.SetAzimuth(15)
            self.ren.ResetCamera()
            self.show()
            self.iren.Initialize()
    ...
    def closeEvent(self, event):
        ...
        rely = MessageBox.question(self, '提示', '确认退出？', MessageBox.Yes, MessageBox.No)
        if rely == MessageBox.Yes:
            MessageBox(
